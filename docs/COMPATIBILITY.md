...
a display setting for the terminal side; version 0.9.39 puts the glyph's percentage beside the terminal
and its bolt, now outlined, at the other end, removes the standard FA26's `OT` badge, widens its `SM`
badge to the whole row and draws its available state yellow, and records the DRS state of every car
with a native DRS component under a generic car family. The Pro adapter, the validity
...
- 0.9.39 one-off checks, not part of the suites: 20 deliberate code mutations of the new badge and
  ...
  For the glyph placement: 20 further mutations were each caught, and 3,072 rendered states (both
  terminal sides) compared draw call by draw call with the build before that change differ only in the
  bolt and the glyph's own text, with every new bolt and percentage position checked.
